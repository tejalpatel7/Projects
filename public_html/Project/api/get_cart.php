...
if ($user_id > 0) {
    $db = getDB();
    $stmt = $db->prepare("SELECT name, c.id as line_id, item_id, quantity, cost, (cost*quantity) as subtotal FROM Cart c JOIN Products i on c.item_id = i.id WHERE c.user_id = :uid");
    try {
